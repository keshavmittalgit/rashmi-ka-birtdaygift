...
8. **Custom Pastel Color Block Creation (HSV Matching)**: When requested to change or add a color-block background color matching a brand hex code:
   - Extract the Hue (H) from the brand color.
   - Scale it to its pastel equivalent by setting Lightness (L) to **88% – 91%** and Saturation (S) to **40% – 75%** (higher saturation for cool colors like blue/lavender, lower for warm colors like yellow/mint).
   - Define the new color token in `globals.css` (both in `@theme` as `--color-block-[name]` and in `:root` as `--block-[name]`) and add the corresponding `.block-[name]` class.
